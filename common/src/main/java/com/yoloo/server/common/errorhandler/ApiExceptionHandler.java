...
import com.yoloo.server.common.mixins.Loggable;
 ...
 * appropriate {@linkplain ErrorResponse}s with a suitable HTTP status code.
 *
 * @author Ali Dehghani
 * @see ErrorCode
 ...
 */
@ControllerAdvice
class ApiExceptionHandler implements Loggable {
   ...
   * given {@code locale}
   */
  private ErrorResponse.ApiError toApiError(
      MessageSource messageSource, ErrorCode errorCode, Locale locale) {
    String message;
    try {
      message = messageSource.getMessage(errorCode.getCode(), null, locale);
    } catch (NoSuchMessageException e) {
      logger()
          .error(
              "Couldn't find any message for {} code under {} locale", errorCode.getCode(), locale);
   ...
   * responses
   *
   * <p>First using the {@linkplain #errorCodeFactory} will find the corresponding {@linkplain
   * ErrorCode} for the given {@code exception}. Then based on the resolved {@linkplain Locale}, a
   * suitable instance of {@linkplain ErrorResponse} with appropriate and localized message will
   * return to the client. {@linkplain ErrorCode} itself determines the HTTP status of the response.
   *
  ...
  public ResponseEntity<ErrorResponse> handleValidationExceptions(
    ...
    List<ErrorResponse.ApiError> apiErrors =
        exception
            .getBindingResult()
            .getAllErrors()
            .stream()
    ...
    return ResponseEntity.badRequest()
        .body(ErrorResponse.ofErrors(HttpStatus.BAD_REQUEST, apiErrors));
  }

  @ExceptionHandler(ConstraintViolationException.class)
  public ResponseEntity<ErrorResponse> handleValidationExceptions2(
  ...
  public ResponseEntity<ErrorResponse> handleValidationExceptions3(
      Exception exception, Locale locale) {
    System.out.println(exception.getClass());

    return ResponseEntity.badRequest()
        .body(ErrorResponse.ofErrors(HttpStatus.BAD_REQUEST, Collections.emptyList()));
  }
}
