...
import com.yoloo.server.core.objectify.util.ObjectifyUtil;
...
public class ObjectifyEntityScanner {
  ...
  public ObjectifyEntityScanner withAdditionalClasses(Class... classes) {
    Stream.of(classes).forEach(additionalClasses::add);
    return this;
  }

  public Set<Class<?>> getEntityClasses() {
    return Stream.concat(getAnnotatedClasses(), additionalClasses.stream()).collect(Collectors.toSet());
  }

  private Stream<Class<?>> getAnnotatedClasses() {
    return ObjectifyUtil.measureAction(() -> {
      final ClassPathScanningCandidateComponentProvider scanner = new ClassPathScanningCandidateComponentProvider(false);
      scanner.addIncludeFilter(new AnnotationTypeFilter(Entity.class, false, false));
      return scanner.findCandidateComponents(basePackage).stream().map(this::beanClass);
    });
  }
