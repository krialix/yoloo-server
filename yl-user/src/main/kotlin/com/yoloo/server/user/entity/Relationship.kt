...
class Relationship(
    ...
    override fun sameIdentityAs(other: Relationship): Boolean {
        return equals(other)
    }
    ...
    companion object {
        ...
        fun getFromKey(relationshipKey: Key<Relationship>): Key<Account> {
            val name = relationshipKey.name
            return Key.create(Account::class.java, name.substring(name.indexOf(':') + 1).toLong())
        }
    }
}